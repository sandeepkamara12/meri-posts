...
const BlogDetails = () => {
  ...
  const { post, relatedPosts, loading, error } = useSelector((state) => state.posts);
  ...
  return (
    <>
      <div className="max-w-4xl w-full mx-auto">
        {
          loading ? <DetailPostLoader layout="" /> :
          <>
      ...
      {/* <input type="number" name="user_post" value={user_id} onChange={(e)=>setUser_id(e.target.value)} /> */}
            ...
            Follow
                ...
                {
                  blog?.tags?.map((tag) => 
                    <Link key={tag} className="capitalize cursor-pointer transition hover:text-white hover:bg-blue-600 inline-flex items-center gap-1.5 py-1.5 px-3 rounded-md text-xs font-medium bg-gray-100 text-gray-800 dark:bg-neutral-800 dark:text-neutral-200" to={`/tag-posts?tag=${tag}`}>{tag}</Link>
                  )
                } 
                  </div>
